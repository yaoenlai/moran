...
class UserApi extends Api {
    ...
    public function register() {
		if($this->verifySign()){
            ...
            //}catch(\Exception $e){
                //$this->ajaxReturn(returnInfo('-5','注册失败!',null,$this->infoType),$this->returnType);
            //}
		}else{
    ...
    public function verify(){
    	if($this->verifySign('secret')){
			...
			//验证Token
			$token = $data['token'];unset($data['token']);//记录并注销原始Token
    ...
    public function greet(){
        if($this->verifySign()){
            ...
            unset($greet_list);unset($greet_list_key);unset($user_list);unset($user_list_key);
            ...
            if(!empty($userModel)){
                foreach($userModel as $key =>$val){
                    ...
                    unset($greetans_arr);unset($greetans);
