            ...
            foreach ($this->_table_column_list as &$column) {
                switch ($column['type']) {
                    case 'status':
                        ...
                        break;
                    case 'byte':
                        $data[$column['name']] = \Common\Util\Transform::formatBytes($data[$column['name']]);
                        break;
                    case 'icon':
                    ...
                    case 'avatar':
                        ...
                        break;
                    case 'equal_scale':
                        $data[$column['name']] = '<img style="max-width:40px;max-height:40px;" src="'.get_cover($data[$column['name']]).'">';
                        break;
                    case 'picture':
                    ...
                    case 'type':
                        ...
                        break;
                    case 'diy_status'://DIY显示状态信息
                        $data[$column['name']] = $column['param'][$data[$column['name']]];
                        break;
                    case 'closing_tag'://span div pre等闭合标签
                        $data[$column['name']] = $column['param']['left'].$data[$column['name']].$column['param']['right'];
                        break;
                    case 'callback': // 调用函数
