...
class InitConfigBehavior extends Behavior {
    ...
    public function run(&$content) {
        ...
        if (!$system_config || APP_DEBUG === true) {
            ...
            $moduleMark = MODULE_MARK;
			if(MODULE_MARK == 'Api'){
				$moduleMark = 'Home';
			}
            $system_config['SESSION_PREFIX'] = ENV_PRE.$moduleMark.'_';  // Session前缀
            $system_config['COOKIE_PREFIX']  = ENV_PRE.$moduleMark.'_';  // Cookie前缀
			...
			if(empty($_COOKIE['session_id'])){
				session_start();
				setcookie("session_id",session_id(),time()+8640000,"/",strstr(C('HOME_DOMAIN'), '.'));
			}else{
		...
		// 如果是前台并且不是Api模块则设置默认控制器层为Api
		if (MODULE_MARK === 'Api' && MODULE_NAME !== 'Api') {
            $system_config['DEFAULT_C_LAYER'] = 'Api';
        }
		
		// 如果是前台并且不是Api模块则设置默认控制器层为Api
		if (MODULE_MARK === 'Shell' && MODULE_NAME !== 'Shell') {
            $system_config['DEFAULT_C_LAYER'] = 'Shell';
        }
		...
		//当前模块的layout 配置
        $layout_path = APP_PATH.MODULE_NAME.'/View/Public/layout.html';
        if(is_file($layout_path)){
            $system_config['MODULE_LAYOUT']  = $layout_path;
        }else{
            $system_config['MODULE_LAYOUT']  = $system_config['HOME_PUBLIC_LAYOUT'];
        }
        ...
        if ($current_theme) {
            ...
            if(is_dir($current_theme_path)){
				...
				//各个模块的layout 模版切换
				$module_layout_path = './Theme/'.$current_theme.'/'.MODULE_NAME.'/Public/layout.html';
				if(is_file($module_layout_path)){
					$system_config['MODULE_LAYOUT']  = $module_layout_path;
				}else{
					$system_config['MODULE_LAYOUT']  = $system_config['HOME_PUBLIC_LAYOUT'];
				}
            }
