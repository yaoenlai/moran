...
class AdminController extends CommonController {
    ...
    protected function _initialize() {
		...
		if($checkMenuAuth['isAdmin'] == '-1'){//对非超级管理员无权限菜单去除
			foreach ($menu_list as $tk => $tv) {
				if(!isset($checkMenuAuth['groupAuth'][$tk])){
					unset($menu_list[$tk]);
				}else{
					foreach ($menu_list[$tk]['_child'] as $lk => $lv) {
						if (!in_array($lv['id'], $checkMenuAuth['groupAuth'][$tk])){//一级菜单没权限直接unset
							unset($menu_list[$tk]['_child'][$lk]);
						}else{
							foreach ($menu_list[$tk]['_child'][$lk]['_child'] as $lk1 => $lv1) {//一级菜单有权限检测二级菜单
								if (!in_array($lv1['id'], $checkMenuAuth['groupAuth'][$tk])){
									unset($menu_list[$tk]['_child'][$lk]['_child'][$lk1]);
								}else{
									foreach ($menu_list[$tk]['_child'][$lk]['_child'][$lk1] as $lk2 => $lv2) {
										if (!in_array($lv2['id'], $checkMenuAuth['groupAuth'][$tk]))
											unset($menu_list[$tk]['_child'][$lk]['_child'][$lk1]['_child'][$lk2]);
									}
								}
							}
						}
						if(!empty($menu_list[$tk]['_child'][$lk]))
							$menu_list[$tk]['_child'][$lk]['_child'] = array_values($menu_list[$tk]['_child'][$lk]['_child']);
					}
				}
				if(!empty($menu_list[$tk]))
					$menu_list[$tk]['_child'] = array_values($menu_list[$tk]['_child']);
			}
			//echo "<pre>";echo MODULE_NAME;dump($checkMenuAuth);
