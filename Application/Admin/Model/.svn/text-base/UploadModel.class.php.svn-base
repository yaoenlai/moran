...
class UploadModel extends Model {
     ...
     * @author zxq
     */
    public function saveUrlfile($urlfiles) {
        // 根目录路径，可以指定绝对路径，比如 /var/www/attached/
        $root_path = './Uploads/';
		$save_path = 'getFile_'.date('Y-m-d').'/';
		if(!is_dir($root_path.$save_path))
			mkdir($root_path.$save_path);
        // 获取文件信息
		if(file_put_contents($root_path.$save_path.basename($urlfiles), file_get_contents($urlfiles))){
			$_FILES = array(
				'size' => filesize($root_path.$save_path.basename($urlfiles)),
				'tmp_name'=> $root_path.$save_path.basename($urlfiles),
				'name'=>basename($urlfiles),
			);
		}
		//dump($_FILES);exit;
        ...
        // 计算文件散列以查看是否已有相同文件上传过
        $reay_file = $_FILES;
        $info['md5']  = md5_file($reay_file['tmp_name']);
        $info['sha1'] = sha1_file($reay_file['tmp_name']);
        $info['size'] = $reay_file['size'];
        $upload = $this->where($info)->find();
        if ($upload) {  // 发现相同文件直接返回
            $return['id']   = $upload['id'];
            $return['name'] = $upload['name'];
            $return['url']  = $upload['real_path'];
            ...
            //$upload_data['type']  = finfo_file($finfo, $reay_file['tmp_name']);
            $upload_data['name']  = $reay_file['name'];
            $upload_data['path']  = '/Uploads/' . $save_path . basename($urlfiles);
            $upload_data['url']   = $info["url"] ? : '';
            $file_ext = strtolower(pathinfo($reay_file['tmp_name'], PATHINFO_EXTENSION));
            $upload_data['ext']   = $file_ext;
            $upload_data['size']  = $info["size"];
            $upload_data['md5']   = $info['md5'];
            $upload_data['sha1']  = $info['sha1'];
            $upload_data['location']  = 'urlfile';
