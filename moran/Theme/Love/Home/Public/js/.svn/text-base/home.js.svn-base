$(function(){
    ...
    */
    ...
    $(document).on('click', '.send-mail-verify', function() {
        var url = Wiera.HOME_PAGE + '/index.php?s=/user/user/send_mail_verify';
        var that = this;
        ...
        if (!filter.test(email)) {
            ...
            $(that).addClass('disabled').prop('disabled', true);
            time(that, 1);
            $.post(url,{'email':email, 'title': title}).success(function(data){
                message = data.info;
                if (data.status == 1) {
    ...
    $(document).on('click', '.send-mobile-verify', function() {
        var url = Wiera.HOME_PAGE + '/index.php?s=/user/user/send_mobile_verify';
        var that = this;
        var title = $(that).attr('data-title');
        ...
        if (!filter.test(mobile)) {
            ...
            $(that).addClass('disabled').prop('disabled', true);
            time(that, 30);
            $.post(url,{'mobile':mobile, 'title': title}).success(function(data){
                if (data.status == 1) {
                    $.alertMessager(data.info, 'success');
                } else {
                    $.alertMessager(data.info, 'danger');
    ...
    function get_location_success(position){
        ...
        $.cookie('oc_latitude', lat, {path: Wiera.VAR_ROOT});
        $.cookie('oc_longitude', lng, {path: Wiera.VAR_ROOT});
    }
    ...
    // 请求定位
    get_location();
    window.setInterval(get_location_start, 30000);
