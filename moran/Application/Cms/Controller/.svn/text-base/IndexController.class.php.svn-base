...
class IndexController extends HomeController {
    ...
    public function index() {
        ...
        if (!$new_doc_type_list) {
            ...
            $con['status'] = 1;
            $con['system'] = 0;
            $doc_type_list = D(D('Index')->moduleName.'/Type')->where($con)->select();

            // 获取字段信息
            $new_doc_type_list = array();
            $attribute_object = D(D('Index')->moduleName.'/Attribute');
            foreach ($doc_type_list as $key => &$val) {
                ...
                foreach ($filter_field_list as $key2 => &$val2) {
                    $val2['options'] = \Common\Util\Think\Str::parseAttr($val2['options']);
        ...
        $this->assign('_index_cate', \Common\Util\Think\Str::parseAttr(C('cms_config.cate')));  // 获取首页栏目自定义配置
    ...
    public function lists($cid) {
        ...
        foreach ($filter_field_list as $key => $val) {
            $val['options'] = \Common\Util\Think\Str::parseAttr($val['options']);
    ...
    public function add() {
        ...
        $field_group = \Common\Util\Think\Str::parseAttr($doc_type['field_group']);
    ...
    public function detail($id) {
        ...
        // 获取评论列表
        $comment_p = I('comment_p') ? : 1;
        $comment_order = I('comment_order') ? : 'sort desc,id desc';
